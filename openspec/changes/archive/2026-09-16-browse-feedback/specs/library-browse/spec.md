...
A single click on the image SHALL zoom it so that it covers its space — the smaller of its two
dimensions filling the space's, so the image overflows along one axis only — and a single click
on the zoomed image SHALL return it to the fit. An image whose cover size is not larger than its
fit SHALL zoom to twice the fit instead, so the click always visibly zooms. The change of scale
SHALL be animated briefly rather than cut. The mouse wheel over the image SHALL scale it in
...
a small movement of the pointer reaches the far end of the image; and a margin of the dark area
SHALL remain around the image at every zoom, so a click beside it still closes the view.
...
- **WHEN** a 4000 by 3000 image is fitted into a 1500 by 900 view and the user clicks it once
- **THEN** the image grows, animated, until it is 1500 pixels wide and taller than the view — not to its 4000 pixels — only the vertical direction can be panned, a margin of the dark area stays beside it, and one more click returns it to the fit

#### Scenario: A small image zooms to twice the fit
- **WHEN** a 400 by 300 image is fitted into a 1500 by 900 view and the user clicks it once
- **THEN** the image is shown covering the view's width; twice the fit is the target only when the cover size would not be larger than the fit

#### Scenario: A wide image covers by height
- **WHEN** a 4000 by 1000 image is fitted into a 1500 by 900 view and the user clicks it once
- **THEN** the image is 900 pixels tall and wider than the view, and only the horizontal direction can be panned
