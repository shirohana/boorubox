...
- **THEN** the image grows, animated, until it is 1500 pixels wide and taller than the view — not to its 4000 pixels — only the vertical direction can be panned, its sides reach the window's edges with no gap, and one more click returns it to the fit
