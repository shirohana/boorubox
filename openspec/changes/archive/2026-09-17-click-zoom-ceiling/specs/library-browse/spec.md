...
ceiling SHALL be adjustable on the settings screen between one and a half and five times the
fit, SHALL start at twice the fit, and SHALL survive a restart. The wheel's own ceiling is not
...
- **WHEN** the ceiling is at its default of twice the fit, a 4000 by 3000 image is fitted into a 1500 by 900 view, and the user clicks it once
...
- **THEN** the image grows to twice the fit — 600 pixels wide by 1800 tall, not the 1500 by 4500 that would cover the width — with dark space beside it, and only the vertical direction can be panned

#### Scenario: A raised ceiling lets the portrait image cover
- **WHEN** the user has set the ceiling to five times the fit and clicks the same 1000 by 3000 image in the same view
- **THEN** the image covers the view's width, 1500 pixels wide and 4500 tall
...
- **THEN** the image is shown at 800 by 600, the ceiling, rather than covering the view's width
...
- **THEN** the image is 3000 pixels wide and 750 tall — twice the fit, where the cover would have been 900 tall — and only the horizontal direction can be panned
