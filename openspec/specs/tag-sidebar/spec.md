...
- **WHEN** the search excludes `dog` and no image in the result carries `dog`
- **THEN** `dog` is still listed, marked as excluded, with a count of zero

#### Scenario: Order
- **WHEN** the list is drawn
- **THEN** the tags the search includes or excludes come first, then the rest by count from highest, and equal counts by name
